...
function responseMessageSend(set, get, data) {
	...
	const friendList = get().friendList.map(item => {
		// Create a copy of the item and modify the copy
		if (item.friend.username === username) {
			return {
				...item, // Spread the existing properties
				preview: data.message.text, // Update preview
				updated: data.message.created, // Update timestamp
				type: data.message.type,
				replied_to:data.message.replied_to
			};
		}
		return item;
	});

	// Move the updated item to the start of the friend list
	...
	// Update the state with the new friendList
	set((state) => ({
		friendList: updatedFriendList
	}));

	// If the message data does not belong to this friend then 
	// don't update the message list, as a fresh messageList will 
...
const useGlobal = create((set, get) => ({
	...
	messageSend: (connectionId, message,type,replied_to) => {
		const socket = get().socket
		socket.send(JSON.stringify({
			source: 'message.send',
			connectionId: connectionId,
	...
	uploadBgImage: async (file) => {
		...
		try {
		  const token = await secure.get('tokens'); // Get the token from storage (e.g., AsyncStorage)
		  ...
		  const response = await axios.post(
			...
			formData,
			{
			  headers: {
				...
				'Authorization': `Bearer ${token.access}`, // Add the token to the request headers
			  },
			}
		  );
		  console.log('Upload successful:', response.data);
		  //alert('Background thumbnail uploaded successfully!');
		  
		} catch (error) {
		  console.error('Upload failed:', error);
		  alert('Failed to upload background thumbnail.');
		}
	  }
