...
const seen = '../assets/seenM.png';
const unseen = '../assets/unseenM.png';
    ...
    <View
      ...
      <View
        ...
        <Menu
          ...
          onDismiss={closeMenu}
          anchor={
            <TouchableOpacity onPress={openMenu} style={{paddingVertical: 5}}>
        ...
        <View
          ...
          {text.seen_at && (
            <Text style={styles.seenTime}>
              {utils.formatTimeChat(text.seen_at)}
            </Text>
          )} */}
                </View>
              ):(
...
function MessagesScreen({navigation, route}) {
  ...
  const markMessagesSeen = useCallback(async () => {
    ...
    try {
      const tokens = await secure.get('tokens');
      const response = await axios.post(
        `https://${ADDRESS}/chat/messages/mark-seen/${connectionId}/`,
        {},
        {
          headers: {
      ...
      console.log('RESPONSE FROM SEEN: ', response);
